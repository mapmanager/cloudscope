...
repository after reading `docs-dev/cloudscope_architecture.md`, `docs-dev/acqstore_api.md`,
`docs-dev/nicewidgets_api.md`, `docs-dev/cloudscope_packaging.md`, and the corresponding
...
The architectural rule, paraphrased from `docs-dev/cloudscope_architecture.md`:
...
- `cloudscope/build_info.py` reads generated `_build_info.py` from the macOS
  ...
  documented in `docs-dev/cloudscope_packaging.md`.
- `docs-dev/cloudscope_packaging.md` is operational (release pipeline, signing,
...
This is documented in `docs-dev/acqstore_api.md`'s "ROI bounds" section, but the
...
- Performance: the current `EChartWidget.apply()` rebuilds the whole options
  ...
  out for the raster viewer in `docs-dev/nicewidgets_api.md`.
- CloudScope side (a new app-level event, suggested in
  `docs-dev/nicewidgets_api.md`):
...
- Every ticket produces a report file under `docs-dev/codex_tickets/`.
