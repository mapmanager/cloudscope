...
This document describes operational packaging and release infrastructure. Runtime architecture is documented separately in `cloudscope_architecture.md`. Single-window desktop and web launch behavior is documented in `cloudscope_single_window_runtime.md`.
...
Runtime modes and env vars for single-window web deployment are documented in:

```text
docs-dev/cloudscope_single_window_runtime.md
```

`docker-compose.yml` is the canonical env reference (`CLOUDSCOPE_REMOTE=1`,
`CLOUDSCOPE_NATIVE=0`).
...
Single-window startup order (`configure_native_window` before `ui.run`, spawn
`__mp_main__` hook, geometry persistence) is documented in:
...
## PyInstaller entry point (required)

Packaged CloudScope builds use PyInstaller / `nicegui-pack`. The application
entry script must follow the multiprocessing guard documented in:
...
- never launch the GUI from __mp_main__
- reload must be False in frozen builds (CLOUDSCOPE_RELOAD=0 in packaging config)
```

Approved pattern in `src/cloudscope/app.py`:
