...
These mutate layout state and in-memory AppConfig. They should not perform hidden saves.
...
## SmartExpansion panels (file list, analysis plot, reference image)

Main-page panels that were previously toggled through AppConfig visibility are wrapped in
`nicewidgets.smart_expansion_widget.SmartExpansion`. Each wrapped view is built with
`initially_visible=False`; the expansion owns MVC lifecycle:

```text
on_open  -> BaseView.show()  -> subscribe + refresh_from_state()
on_close -> BaseView.hide()  -> unsubscribe_events()
```

The `ANALYSIS_REFERENCE` horizontal splitter is preserved between the analysis-plot and
reference-image SmartExpansion panes so users can still resize those sections vertically.
...
See `docs-dev/cloudscope_project_rules.md` for repository layout, package boundary, API, testing, and development workflow rules.
