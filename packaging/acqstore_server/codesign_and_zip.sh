#!/usr/bin/env bash
# Codesign "AcqStore Server.app" and create the pre-notarization zip.
# Pattern copied from packaging/macos/codesign_and_zip.sh (config-driven).
...
if [[ ! -f "$APP_MAIN_BIN" ]]; then
  echo "ERROR: Main binary not found: $APP_MAIN_BIN" >&2
  exit 2
fi
