...
def main_uvicorn() -> None:
    ...
    logger.info('v2 health http://%s:%s/api/v2/health', host, port)
    logger.info('v2 demo http://%s:%s/demo/v2/', host, port)
    logger.info('log file %s', log_file_path())
    print(f'[acqstore_server] {APP_NAME} v{APP_VERSION}')
    print(f'[acqstore_server] listening http://{host}:{port}')
    print(f'[acqstore_server] v2 demo http://{host}:{port}/demo/v2/')
