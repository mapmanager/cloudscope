...
def _reference_snapshot_from_oir_reference(ref: Any) -> ReferenceImageSnapshot:
    """Build a :class:`ReferenceImageSnapshot` while ``oirfile.OirFile`` is still open."""
...
def _png_bytes_from_uint8_yx(arr_yx: np.ndarray) -> bytes:
    """Encode a single-channel ``uint8`` ``(Y, X)`` image as PNG bytes."""
    if arr_yx.ndim != 2:
        raise ValueError(f"Expected 2D uint8 array for PNG, got shape {arr_yx.shape}")
    if arr_yx.dtype != np.uint8:
        raise ValueError(f"Expected dtype uint8 for PNG, got {arr_yx.dtype}")
    bio = BytesIO()
    Image.fromarray(arr_yx, mode="L").save(bio, format="PNG")
    return bio.getvalue()
...
class ImageLoaderBase:
    ...
    Display caches (``display_uint8_volume``, PNG bytes) are intended for use from a
    **single thread** per loader instance (typical NiceGUI per-client handler). They are
    ...
    def __init__(self, path: str, header: ImageHeader | None = None) -> None:
        ...
        self._init_lazy_display_cache_attrs()
        if header is None:
            self._header = self.read_header()
        else:
            self._header = header

    def _init_lazy_display_cache_attrs(self) -> None:
        """Reset lazy GUI/display caches (primary image uint8 + per-slice PNG)."""
        self._display_uint8_volume: np.ndarray | None = None
        self._slice_png_bytes: dict[tuple[int, int, int], bytes] = {}
    ...
    def unload_image_data(self) -> None:
        """Drop cached primary pixel data and primary display caches; header unchanged."""
        self._img_data = None
        self._display_uint8_volume = None
        self._slice_png_bytes.clear()

    def unload_reference_data(self) -> None:
        """Drop reference-image caches. Subclasses with references override."""
        pass

    def unload_all_cached_data(self) -> None:
        """Clear primary pixels, primary display caches, and reference caches."""
        self.unload_image_data()
        self.unload_reference_data()

    def reference_snapshot(self) -> ReferenceImageSnapshot | None:
        """Return a frozen reference snapshot when the format provides one; else ``None``."""
        return None
    ...
    def display_uint8_volume(self) -> np.ndarray:
        """Full-volume ``uint8`` for GUI using :meth:`array_for_plotly_display` (lazy, once)."""
        if self._display_uint8_volume is None:
            raw = self.load_image_data()
            u8 = self.array_for_plotly_display(raw)
            u8.setflags(write=False)
            self._display_uint8_volume = u8
        return self._display_uint8_volume

    def slice_display_png_bytes(self, channel: int = 0, z: int = 0, t: int = 0) -> bytes:
        """Lazy PNG bytes for one ``(Y, X)`` slice at ``channel`` / ``z`` / ``t`` (greyscale)."""
        key = (channel, z, t)
        if key not in self._slice_png_bytes:
            u8vol = self.display_uint8_volume()
            yx = self.yx_slice_from_volume(
                u8vol,
                self._header.dims,
            ...
            self._slice_png_bytes[key] = _png_bytes_from_uint8_yx(yx)
        return self._slice_png_bytes[key]
...
def _init_loader_from_stream(
    ...
    if header is None:
        ...
        inst._header = header
    inst._init_lazy_display_cache_attrs()
    return inst
...
class MyOirImage(ImageLoaderBase):
    ...
    def _init_lazy_display_cache_attrs(self) -> None:
        super()._init_lazy_display_cache_attrs()
        self._oir_ref_snapshot_loaded = False
        self._oir_reference_snapshot: ReferenceImageSnapshot | None = None
        self._oir_ref_display_uint8: np.ndarray | None = None
        self._oir_ref_png_bytes: dict[tuple[int, int, int], bytes] = {}
    ...
    def reference_snapshot(self) -> ReferenceImageSnapshot | None:
        ...
        """
        if self._oir_ref_snapshot_loaded:
            return self._oir_reference_snapshot
        with self._open_oir() as oir:
            ref = oir.reference
            if ref is None:
                self._oir_reference_snapshot = None
            else:
                self._oir_reference_snapshot = _reference_snapshot_from_oir_reference(ref)
        self._oir_ref_snapshot_loaded = True
        return self._oir_reference_snapshot

    def reference_display_uint8_volume(self) -> np.ndarray | None:
        """Full reference image as ``uint8`` via :meth:`array_for_plotly_display`, or ``None``."""
        snap = self.reference_snapshot()
        if snap is None:
            return None
        if self._oir_ref_display_uint8 is None:
            u8 = self.array_for_plotly_display(snap.array)
            u8.setflags(write=False)
            self._oir_ref_display_uint8 = u8
        return self._oir_ref_display_uint8

    def reference_slice_png_bytes(self, channel: int = 0, z: int = 0, t: int = 0) -> bytes:
        """PNG bytes for one reference ``(Y, X)`` slice (lazy, cached per ``channel,z,t``)."""
        snap = self.reference_snapshot()
        if snap is None:
            raise ValueError("No OIR reference image for this file")
        key = (channel, z, t)
        if key not in self._oir_ref_png_bytes:
            u8 = self.reference_display_uint8_volume()
            assert u8 is not None
            yx = self.yx_slice_from_volume(
                u8, snap.dims, snap.num_channels, channel, z=z, t=t
            )
            self._oir_ref_png_bytes[key] = _png_bytes_from_uint8_yx(yx)
        return self._oir_ref_png_bytes[key]

    def unload_reference_data(self) -> None:
        """Clear cached OIR reference snapshot and reference display buffers."""
        self._oir_ref_snapshot_loaded = False
        self._oir_reference_snapshot = None
        self._oir_ref_display_uint8 = None
        self._oir_ref_png_bytes.clear()
