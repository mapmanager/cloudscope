"""Tests for :class:`AcqImageDataController`."""
...
class _FakeAcqImage:
    """Minimal stand-in for :class:`AcqImage` lazy-load orchestration tests."""
...
def test_clear_selection_invokes_callback_synchronously() -> None:
    ctrl = AcqImageDataController()
    seen: list[str] = []
    ctrl.ensure_loaded_for_selection(None, None, on_complete=lambda: seen.append('done'))
    assert seen == ['done']
...
def test_demo_file_without_acq_image_invokes_callback_without_load() -> None:
    ctrl = AcqImageDataController()
    seen: list[str] = []
    ctrl.ensure_loaded_for_selection('demo.tif', None, on_complete=lambda: seen.append('done'))
    assert seen == ['done']
...
def test_already_loaded_file_invokes_callback_synchronously() -> None:
    ...
    seen: list[str] = []
    ctrl.ensure_loaded_for_selection('a.oir', acq, on_complete=lambda: seen.append('done'))
...
def test_same_file_reselection_from_pool_invokes_without_reload() -> None:
    ...
    acq = _FakeAcqImage(loaded=True)
    seen: list[str] = []
    ctrl.ensure_loaded_for_selection('a.oir', acq, on_complete=lambda: seen.append('first'))
    ctrl.ensure_loaded_for_selection('a.oir', acq, on_complete=lambda: seen.append('second'))
    assert acq.load_calls == 0
...
def test_stale_load_is_ignored_after_fast_file_switch(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    async def _run() -> None:
        ctrl.ensure_loaded_for_selection('a.oir', acq_a, on_complete=lambda: seen.append('a'))
        task_a = pending_tasks[0]
        ctrl.ensure_loaded_for_selection('b.oir', acq_b, on_complete=lambda: seen.append('b'))
...
def test_load_failure_does_not_invoke_callback(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    acq.load_lazy_data = MagicMock(side_effect=_boom)  # type: ignore[method-assign]
