"""Tests for ``SelectionFooterView`` and footer display helpers."""
...
from cloudscope.gui.selection_footer_view import SelectionFooterView, footer_display_values
...
def test_selection_footer_view_reacts_to_file_then_channel_and_roi() -> None:
    """View handlers update label text without calling ``build()`` (no NiceGUI slot)."""
    bus = EventBus()
    view = SelectionFooterView(bus)
...
def test_selection_footer_view_clear_file_resets_display_to_placeholders() -> None:
    bus = EventBus()
    view = SelectionFooterView(bus)
    view._file_label = _TextSink()
    view._channel_label = _TextSink()
