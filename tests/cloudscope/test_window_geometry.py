...
def test_attach_registers_move_and_resize_handlers(tmp_path) -> None:
    ...
    window = _FakeWindow()
    tracker = WindowGeometryTracker(app_config, window)
...
def test_persist_saves_once(tmp_path, monkeypatch: pytest.MonkeyPatch) -> None:
    app_config = AppConfig.ephemeral(config_path=tmp_path / 'config.json')
    window = _FakeWindow(x=11, y=22, width=333, height=444)
    tracker = WindowGeometryTracker(app_config, window)
    save = MagicMock()
    monkeypatch.setattr(app_config, 'save', save)
