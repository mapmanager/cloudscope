...
def test_on_copy_results_copies_tsv(monkeypatch) -> None:
    ...
    view = DiameterAnalysisView(event_bus=EventBus())
    bookkeeping = pd.DataFrame({"channel": [0], "roi_id": [1], "diameter": [3.2]})
    analysis = _FakeAnalysis(table=pd.DataFrame({"diameter": [3.2]}), bookkeeping=bookkeeping)
    view._selected_analysis = lambda: analysis  # type: ignore[method-assign]

    view._on_copy_results_clicked()

    assert "\t" in captured["text"]
    assert "diameter" in captured["text"]
