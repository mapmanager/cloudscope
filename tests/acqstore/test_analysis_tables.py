...
def test_table_with_bookkeeping_adds_channel_and_roi() -> None:
    ...
    analysis = VelocityAnalysis(channel=3, roi_id=4)
...
def test_save_and_load_results_df(tmp_path: Path) -> None:
    ...
    a = VelocityAnalysis(channel=0, roi_id=1)
    a.result.table = pd.DataFrame({"time_s": [0.0], "velocity": [1.0]})
    b = VelocityAnalysis(channel=1, roi_id=2)
    b.result.table = pd.DataFrame({"time_s": [0.0], "velocity": [2.0]})
    analysis_set.add(a)
    analysis_set.add(b)
    analysis_set.save_results_df(source)

    assert (tmp_path / "myfile.tif.velocity.csv").exists()
    ...
    assert loaded_a.get_column("velocity") == [1.0]
...
def test_save_results_df_deletes_stale_csv_when_no_rows_remain(tmp_path: Path) -> None:
    ...
    source = tmp_path / "myfile.tif"
    source.write_text("dummy")
    stale = tmp_path / "myfile.tif.velocity.csv"
