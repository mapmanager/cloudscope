...
def test_analysis_key_excludes_file_id() -> None:
    ...
    key = AnalysisKey("velocity", channel=1, roi_id=2)

    assert key.analysis_name == "velocity"
...
def test_analysis_key_csv_filename_preserves_source_extension() -> None:
    ...
    key = AnalysisKey("velocity", channel=0, roi_id=1)

    assert key.csv_filename("myfile.tif") == "myfile.tif.velocity.csv"
