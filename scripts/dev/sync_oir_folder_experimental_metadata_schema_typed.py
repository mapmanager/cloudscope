...
def _csv_value_to_metadata_value(csv_column: str, raw_value: str) -> Any:
    ...
    if csv_column == "Order":
        ...
        if float_value != int_value:
            raise ValueError(f"Order must be an integer-compatible value, got {value!r}")
    ...
    for oir_path in oir_paths:
        ...
        try:
            for csv_column, metadata_key in CSV_TO_METADATA.items():
                ...
                converted_values[metadata_key] = _csv_value_to_metadata_value(
                    csv_column,
                    raw_value,
                )
        except ValueError as error:
            skip_messages.append(
                f"SKIP invalid metadata value for OIR: {oir_path} ({error})"
            )
            continue

        for metadata_key, new_value in converted_values.items():
