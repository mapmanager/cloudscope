...
def _copy_file(src: Path, dst: Path, skip_messages: list[str]) -> bool:
    if dst.exists() and not OVERWRITE:
        skip_messages.append(f"SKIP exists: {dst}")
        return False
...
def sync_oir_and_analysis_to_tif_structure(
    ...
    if duplicate_tif_stems:
        warning_messages.append("WARNING: Duplicate TIF stems found in Folder 1.")
        warning_messages.append("These stems are unsafe and will not be copied.")

        for stem, tif_paths in sorted(duplicate_tif_stems.items()):
            warning_messages.append("")
            warning_messages.append(f"Duplicate stem: {stem}")
            warning_messages.append("  Folder 1 TIF files:")
            for tif_path in sorted(tif_paths):
                warning_messages.append(f"    {tif_path}")

            warning_messages.append("  Folder 2 matching OIR files:")
            matching_oir_paths = sorted(oir_index.get(stem, []))
            if matching_oir_paths:
                for oir_path in matching_oir_paths:
                    warning_messages.append(f"    {oir_path}")
            else:
                warning_messages.append("    No matching OIR files found.")
    ...
    if COPY_OIR_FILES:
        ...
        for stem, tif_path in sorted(safe_tif_items.items()):
            ...
            if not matching_oir_paths:
                skip_messages.append(f"Missing OIR for TIF stem: {stem}")
                continue

            if len(matching_oir_paths) > 1:
                skip_messages.append(f"SKIP ambiguous OIR matches for stem: {stem}")
                for oir_path in sorted(matching_oir_paths):
                    skip_messages.append(f"    {oir_path}")
                continue
            ...
            if _copy_file(oir_path, destination_path, skip_messages):
                copied_oir_count += 1
        ...
        for stem, tif_path in sorted(safe_tif_items.items()):
            ...
            if not destination_oir_path.exists():
                skip_messages.append(
                    f"SKIP analysis; destination OIR does not exist: {destination_oir_path}"
                )
                continue
            ...
            if not analysis_candidates:
                skip_messages.append(f"No analysis files found for TIF: {tif_path}")
                continue

            for analysis_path in analysis_candidates:
                if ".tif." not in analysis_path.name:
                    skip_messages.append(
                        f"SKIP analysis filename without '.tif.': {analysis_path}"
                    )
                    continue
                ...
                if _copy_file(analysis_path, destination_path, skip_messages):
